...
so qualification uses the minimum fixed topology, a six-hour dump/two-hour
...
direct-only run. `TTL_HOURS` defaults to 6 because dump bootstrap requires at
least six hours, while snapshot replay may explicitly use 2. It remains capped
...
`up` uses an 18,000-second command watchdog, a separate 2,400-second failure
cleanup watchdog, a 300-second lease-transition margin, a 20,700-second lease,
a 359-minute job, and a six-hour
short-lived role session/TTL. Its data-bootstrap SSM command is bounded at
12,600 seconds and its association waiter at 12,900 seconds so result propagation
cannot race the command timeout. The 1,800-second pre-bootstrap and 3,000-second
post-bootstrap figures are planning allocations inside the outer watchdog, not
independent stage timers.
...
`MaxSessionDuration` to remain 21,600 seconds, binds the lock's S3-server
`LastModified` and `VersionId`, and creates a create-only S3 clock receipt before
requiring at least 21,900 seconds of AWS-authoritative elapsed time. The lock
