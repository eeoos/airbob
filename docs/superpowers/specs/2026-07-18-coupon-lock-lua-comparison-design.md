...
이 API는 Lua 경로에만 필요하다. DB 쿠폰 설정을 Redis로 복제하며, 다음 조건을 모두 만족해야 한다.

- `totalQuantity`가 `null`인 무제한 쿠폰이거나 양수인 유한 쿠폰이다.
- 쿠폰이 활성 상태다.
- 발급 기간이 유효하고 아직 종료되지 않았다.
...
준비 성공 시 DB의 `coupon.redis_stock_prepared_at`에도 준비 이력을 영구 기록한다. 이 값은 Redis TTL 만료나 재시작 뒤에도 발급 설정 불변성과 URL 경로 분리를 유지하는 기준이다. 준비와 락 발급은 같은 쿠폰 DB 행을 `PESSIMISTIC_WRITE`로 읽어 발급 시작 경계에서 두 트랜잭션을 직렬화한다. 준비 이력이 있는 쿠폰은 락 경로를 거부하고, Lua의 DB 저장 경로는 준비 이력이 없는 쿠폰을 거부한다. 따라서 요청 enum 없이도 같은 쿠폰 ID에 락과 Lua를 섞어 재고를 초과 발급할 수 없다.

Redis 준비 직후 DB 커밋이 실패해 영구 이력이 남지 않는 경우도 락 경로가 Redis 키 존재 여부를 확인해 발급을 거부한다. 이 상태는 잘못된 경로로 발급하기보다 해당 캠페인을 fail-closed 상태로 두며, 운영 복구 절차를 수행하거나 새 쿠폰 캠페인을 생성해야 한다.
...
Redis 준비가 끝난 뒤에는 DB의 영구 준비 이력을 기준으로 `totalQuantity`, 활성 상태, 발급 기간을 수정할 수 없다. Redis 키 존재 여부도 준비 도중의 비정상 종료를 방어하는 보조 기준으로 확인한다. 이벤트 종료 후 새 쿠폰을 만드는 방식으로 변경한다. 긴급 중단과 실행 중 설정 변경은 별도 후속 설계 대상으로 둔다.
...
1. `coupon:{couponId}:lock` Redisson 락을 획득한다.
2. DB의 Redis 준비 이력이 없는 쿠폰인지 확인하고 `CouponIssueTransactionService.issueUnderLock()`을 호출한다.
...
- `issueUnderLock()`: 쿠폰 행을 쓰기 잠금으로 읽어 Redis 준비와 직렬화하고, DB 준비 이력 또는 Redis 준비 키가 없는지 확인한 뒤 활성 상태, 발급 기간, 재고, 회원 중복을 검증해 발급한다.
- `persistApprovedIssue()`: Redis에서 이미 검증된 당첨 요청의 `issuedQuantity`를 증가시키고 `MemberCoupon`을 저장한다.

`issueUnderLock()`은 `redisStockPreparedAt`이 있거나 Redis 준비 키가 남은 Lua 캠페인을 거부하고, `persistApprovedIssue()`는 영구 준비 이력이 없는 락 캠페인을 거부한다. 별도 URL이라는 운영 규칙을 저장소 상태로도 강제한다.
...
반복 발급 요청은 Redisson의 script cache를 켜서 최초 로드 이후 `EVALSHA` 경로를 사용한다. 매 요청마다 Lua 본문 전체를 전송하는 비용이 비교 결과에 섞이지 않게 한다.
...
관리자 prepare에서 Redis 쓰기 뒤 DB 준비 이력 커밋 전에 프로세스가 종료되거나 커밋이 실패하면 Redis 키만 남을 수 있다. 락 발급은 이 키를 확인해 혼용을 막지만 해당 쿠폰은 자동 복구되지 않는다. 1차 운영에서는 새 캠페인 생성 또는 검증된 수동 복구 절차로 처리한다.
...
모든 계측 호출은 예외 비전파 경계로 감싼다. MeterRegistry 장애가 Lua 승인 결과, DB 보상 판단, 락 해제 여부를 바꾸어서는 안 된다.
...
- 각 요청에 서로 다른 회원의 세션을 하나씩 배정한다.
- 각 실행 전 DB와 Redis 상태를 초기화하고 Lua 쿠폰만 준비 API를 호출한다.
- 연결과 JIT 워밍업은 별도의 폐기용 쿠폰으로 수행해 측정용 재고를 소모하지 않는다. 이후 실행 순서를 교차해 순서 편향을 줄인다.
...
락 경로는 DB 준비 이력이 없는 일반 쿠폰에 대해 비정상 Redis prepare 잔존 여부를 확인하는 `countExists` 1회를 수행한다. 이 호출은 같은 쿠폰에 두 URL을 섞지 못하게 하는 fail-closed 보장의 일부이므로 측정 수치에 포함하고 결과 보고서에 기록한다.
...
- 준비 API는 신규 유한·무제한 쿠폰을 한 번만 준비한다.
- 발급이 시작됐어도 종료 전이고 DB 발급 이력이 모두 0이면 준비한다.
...
- 발급 시작 경계에서 Redis prepare와 락 발급이 겹쳐도 한 경로만 허용된다.
