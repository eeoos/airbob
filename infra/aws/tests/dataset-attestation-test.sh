...
write_release() {
  ...
  for migration_version in 10 11 12 13 14 15 16 17 1 2 3 4 5 6 7 8 9; do
  ...
  : > "$migration_stream"
  for migration_version in $(seq 1 17); do
  ...
  jq -n --arg migrationDigest "$migration_digest" '
    {
      ...
      schema: {flywayVersion: "17", migrationDigest: $migrationDigest}
    }
...
write_fake_mysql() {
  ...
  printf '%s\n' '  *"COUNT(*) AS history_rows"*) printf "%b\n" "${AIRBOB_FAKE_FLYWAY_SUMMARY:-17\\t17\\t17\\t0}" ;;' >> "$tmp_dir/bin/mysql"
  printf '%s\n' '  *"SELECT installed_rank, COALESCE(version,"*) for rank in $(seq 1 17); do printf "%s\t%s\tmigration %s\tSQL\tV%s__migration_%s.sql\t%s\t1\n" "$rank" "$rank" "$rank" "$rank" "$rank" "$rank"; done ;;' >> "$tmp_dir/bin/mysql"
  ...
  printf '%s\n' '  *"SELECT COUNT(*) FROM "*"flyway_schema_history"*) printf "17\n" ;;' >> "$tmp_dir/bin/mysql"
  printf '%s\n' '  *"SELECT COUNT(*) FROM "*"outbox"*) printf "%s\n" "${AIRBOB_FAKE_OUTBOX_COUNT:-0}" ;;' >> "$tmp_dir/bin/mysql"
...
expect_failure v16-history run_capture \
  "$tmp_dir/release" "$tmp_dir/v16.json" \
  AIRBOB_FAKE_FLYWAY_SUMMARY='16\t17\t17\t0'

expect_failure history-row-mismatch run_capture \
  "$tmp_dir/release" "$tmp_dir/history-row-mismatch.json" \
  AIRBOB_FAKE_FLYWAY_SUMMARY='17\t18\t17\t0'
