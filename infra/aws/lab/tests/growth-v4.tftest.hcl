...
variables {
  ...
  dataset_release            = "korea-growth-v4-c8c75883908247ef-aws-r1"
  dataset_manifest_sha256    = filesha256("tests/fixtures/growth-v4-aws-manifest.json")
...
override_data {
  ...
  override_during = plan
  values = {
    body = file("tests/fixtures/growth-v4-aws-manifest.json")
  }
}

run "qualify_v28_source_with_historical_inventory" {
  command = plan
  assert {
    condition = (
      ...
      strcontains(local.bootstrap_data_command, "bootstrap-growth-v4-aws.py") &&
      length(local.bootstrap_data_command) < 45000
    )
    error_message = "V28 preparation must select its bounded helper and exact MySQL engine without v2 spec or app resources."
  }
}
