...
output "phase4_contract" {
  ...
  value = {
    ...
    instance_type                      = local.application_infra_enabled ? module.app_asg[0].contract.instance_type : "c6i.large"
    capacity = local.application_infra_enabled ? {
      min     = module.app_asg[0].contract.min
      desired = module.app_asg[0].contract.desired
      max     = module.app_asg[0].contract.max
    } : local.app_capacity
    app_subnet_count                    = local.application_infra_enabled ? module.app_asg[0].contract.subnet_count : length(local.app_subnet_ids)
    app_availability_zones              = local.app_availability_zones
    scaling_policy_count                = local.application_infra_enabled ? module.app_asg[0].contract.scaling_policy_count : 0
    request_count_per_target_per_minute = local.application_infra_enabled ? module.app_asg[0].contract.request_target_per_minute : var.request_count_per_target_per_minute
    cpu_target_percent                  = local.application_infra_enabled ? module.app_asg[0].contract.cpu_target_percent : 50
    default_instance_warmup             = local.application_infra_enabled ? module.app_asg[0].contract.default_instance_warmup : 180
    runtime_revision                    = local.app_runtime_revision
    alb_arn                             = local.application_infra_enabled ? module.alb[0].arn : null
    alb_dns_name                        = local.application_infra_enabled ? module.alb[0].dns_name : null
    alb_zone_id                         = local.application_infra_enabled ? module.alb[0].zone_id : null
    alb_security_group_id               = local.application_infra_enabled ? module.security.security_group_ids["alb"] : null
    target_group_arn                    = local.application_infra_enabled ? module.alb[0].target_group_arn : null
    auto_scaling_group_name             = local.application_infra_enabled ? module.app_asg[0].name : null
    alb_https_only                      = local.application_infra_enabled ? module.alb[0].contract.https_only : true
    alb_stickiness_enabled              = local.application_infra_enabled ? module.alb[0].contract.stickiness_enabled : false
    ...
    refresh = local.application_infra_enabled ? module.app_asg[0].contract.refresh : {
