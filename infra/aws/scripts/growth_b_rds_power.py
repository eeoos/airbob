...
def observe(aws, request):
    ...
    # StartDBInstance can pass through monitoring configuration and an RDS reboot.
    need(row.get('DBInstanceStatus') in {'available', 'stopped', 'starting', 'stopping',
        'configuring-enhanced-monitoring', 'rebooting'}, 'RDS_STATE_UNSUPPORTED')
...
def execute(request, aws=None, *, now=time.time, sleep=time.sleep):
    ...
    while True:
        ...
        if state == desired:
            ...
            return proof
        waiting = {'starting', 'configuring-enhanced-monitoring', 'rebooting'} if desired == 'available' else {'stopping'}
