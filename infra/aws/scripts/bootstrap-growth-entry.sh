...
[[ "$AIRBOB_DATASET_RELEASE" =~ ^korea-growth-v3-[0-9a-f]{16}-aws(-r[1-9][0-9]{0,2})?$ ||
   "$AIRBOB_DATASET_RELEASE" =~ ^korea-growth-v4-[0-9a-f]{16}-aws-r[1-9][0-9]{0,2}$ ]]
...
if [[ "$(jq -r '.releaseKind' "$manifest")" == growth-v4-aws-qualification ]]; then
  python3 "$helper_dir/bootstrap-growth-v4-aws.py" --manifest "$manifest"
  exit 0
fi
