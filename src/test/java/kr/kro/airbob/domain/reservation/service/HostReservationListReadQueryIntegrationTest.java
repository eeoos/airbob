...
import kr.kro.airbob.domain.reservation.dto.ReservationResponse.HostReservationInfo;
import kr.kro.airbob.domain.reservation.dto.ReservationResponse.HostReservationInfos;
...
@Import({JpaAuditingConfig.class, QueryDslConfig.class,
	...
	HostReservationListReadQueryIntegrationTest.ReadTestConfig.class})
@DirtiesContext(classMode = DirtiesContext.ClassMode.AFTER_CLASS)
class HostReservationListReadQueryIntegrationTest {
	...
	private static final MySQLContainer<?> MYSQL = new MySQLContainer<>("mysql:8.4.11")
		.withDatabaseName("airbobdb_host_reservation_list");
	...
	void preservesFiltersOwnershipTiedCursorsAndLastPage(ReservationFilterType filter) {
		...
		for (long id = 40; id <= 43; id++) {
			insertReservation(id, id % 2 == 0 ? 30 : 31, id % 2 == 0 ? 20 : 21, status, checkIn);
		}
		jdbc.update("UPDATE reservation SET created_at = ? WHERE id = 40", CREATED_AT.minusSeconds(1));
		insertReservation(44, 32, 20, status, checkIn);
		insertReservation(45, 30, 20, ReservationStatus.PAYMENT_PENDING, checkIn);
		insertReservation(46, 30, 20, ReservationStatus.PAYMENT_PROCESSING, checkIn);
		...
		HostReservationInfos first = find(10L, filter, null, 2);
		assertThat(first.reservations()).extracting(HostReservationInfo::reservationUid)
			.containsExactly(uid(43), uid(42));
		...
		assertThat(first.reservations().getFirst().guest().nickname()).isEqualTo("다른 게스트");
		assertThat(first.reservations().getFirst().guest().thumbnailImageUrl()).isNull();
		assertThat(first.reservations()).allSatisfy(row -> {
			...
			assertThat(row.totalPrice()).isEqualTo(100001L);
			assertThat(row.currency()).isEqualTo("KRW");
			assertThat(row.guestCount()).isEqualTo(2);
		...
		HostReservationInfos second = find(10L, filter, cursor, 2);
		assertThat(second.reservations()).extracting(HostReservationInfo::reservationUid)
			.containsExactly(uid(41), uid(40));
		assertThat(second.pageInfo()).isEqualTo(new PageInfo(false, null, 2));
		HostReservationInfos empty = find(10L, filter, new CursorData(40L, CREATED_AT.minusSeconds(1)), 2);
		assertThat(empty.reservations()).isEmpty();
		assertThat(empty.pageInfo()).isEqualTo(new PageInfo(false, null, 0));
	}

	@Test
	void preservesFrontendJsonContractWithoutExposingInternalCursorId() throws Exception {
		insertReservation(41, 30, 20, ReservationStatus.CONFIRMED, LocalDate.parse("2026-11-01"));
		HostReservationInfos result = find(10L, ReservationFilterType.UPCOMING, null, 20);

		try (var fixture = new ClassPathResource("contracts/host-reservation-list.json").getInputStream()) {
	...
	void cursorSurvivesDeletedBoundaryAndNewerReservationInsertion() {
		...
		HostReservationInfos first = find(10L, ReservationFilterType.UPCOMING, null, 1);
		assertThat(first.reservations()).extracting(HostReservationInfo::reservationUid).containsExactly(uid(42));
		CursorData cursor = cursorDecoder.decode(first.pageInfo().nextCursor(), CursorData.class);
		...
		HostReservationInfos next = find(10L, ReservationFilterType.UPCOMING, cursor, 2);
		assertThat(next.reservations()).extracting(HostReservationInfo::reservationUid)
			.containsExactly(uid(41), uid(40));
		assertThat(next.pageInfo()).isEqualTo(new PageInfo(false, null, 2));
	}

	@Test
	void anotherHostCannotSeeReservationsOfThisHost() {
		insertReservation(41, 30, 20, ReservationStatus.CONFIRMED, LocalDate.parse("2026-11-01"));

		HostReservationInfos result = find(11L, ReservationFilterType.UPCOMING, null, 20);
		assertThat(result.reservations()).isEmpty();
		assertThat(result.pageInfo()).isEqualTo(new PageInfo(false, null, 0));
	}

	@Test
	void omittedFilterKeepsOwnershipAndExcludesPendingPayments() {
		long id = 40;
		...
		insertReservation(55, 32, 20, ReservationStatus.CONFIRMED, LocalDate.parse("2026-11-01"));

		HostReservationInfos result = find(10L, null, null, 20);
		assertThat(result.reservations()).extracting(HostReservationInfo::status).containsExactlyInAnyOrder(
			ReservationStatus.CONFIRMED, ReservationStatus.CANCELLATION_PENDING,
			ReservationStatus.CANCELLATION_FAILED, ReservationStatus.CANCELLED, ReservationStatus.EXPIRED);
		assertThat(result.pageInfo()).isEqualTo(new PageInfo(false, null, 5));
	}

	private HostReservationInfos find(Long hostId, ReservationFilterType filter, CursorData cursor, int size) {
		entityManager.flush();
		entityManager.clear();
		Statistics statistics = entityManagerFactory.unwrap(SessionFactory.class).getStatistics();
		statistics.clear();
		sqlCapture.statements.clear();
		HostReservationInfos result = service.findHostReservations(hostId,
		...
		assertThat(sql.substring("select ".length(), sql.indexOf(" from ")).split(",")).hasSize(17);
		assertThat(statistics.getEntityLoadCount()).isZero();
		assertThat(sql).doesNotContain(" join address ", "payment_transaction", " join payment ",
			".description", ".message", ".email", ".role", ".updated_at", ".created_by");
		return result;
	}
	...
	private static String uid(long id) {
		return "30000000-0000-4000-8000-%012d".formatted(id);
	}
