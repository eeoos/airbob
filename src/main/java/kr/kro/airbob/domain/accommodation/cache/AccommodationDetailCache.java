...
public class AccommodationDetailCache {
	...
	private AccommodationDetailSnapshot loadWithoutCache(
		...
		// 대신 같은 JVM에서 시작된 우회 DB 조회만 CompletableFuture로 공유
		...
		CompletableFuture<AccommodationDetailSnapshot> existing =
			localLoads.putIfAbsent(accommodationId, newLoad); // key 존재 시 기존 값 반환/ 미존재 시 newLoad를 넣고 null 반환
		if (existing != null) { // key 존재하는 경우
