#!/usr/bin/env python3
"""Bounded representative HTTP reads of an already running ordinary local Airbob.
...
def read_inputs(args):
    ...
    need(isinstance(environment, str) and re.fullmatch(r'local:[a-zA-Z0-9][a-zA-Z0-9_.-]{0,62}:' + dataset, environment),
         'EXACT_LOCAL_ACCOUNT_ENVIRONMENT_REQUIRED')
    ...
    for row in credentials:
        ...
        by_id[member] = row
    return {'datasetId': dataset, 'baseUrl': base_url(args.base_url), 'selected': selected,
            ...
            'binding': {'consumerManifestSha256': args.consumer_manifest_sha256,
                        ...
                        'qualifiedAppJarSha256': consumer['appJarSha256']}}
...
def run(args, *, session_factory=HttpSession):
    ...
    evidence = {'schemaVersion': 1, 'kind': 'global-b-dev-representative-http-reads',
                'state': STATE if passed else 'LOCAL_B_REPRESENTATIVE_HTTP_READS_FAILED', 'passed': passed,
...
def main(argv=None):
    ...
    try:
        ...
        print(json.dumps({'state': 'LOCAL_B_REPRESENTATIVE_HTTP_READS_REJECTED', 'failureCode': error.code}))
        return 2
    except Exception:
        print(json.dumps({'state': 'LOCAL_B_REPRESENTATIVE_HTTP_READS_REJECTED', 'failureCode': 'UNCLASSIFIED_FAILURE'}))
        return 2
