...
function operation(datasetSize, overrides = {}) {
  return {
    operation_name: 'expired-reservation-cleanup-before',
    outcome: 'SUCCESS',
...
function payload(datasetSize, overrides = {}) {
  return {
    success: true,
    data: {
      ...
      operation: operation(datasetSize),
      ...overrides,
    },
  };
}
...
export default function () {
  ...
  check(null, {
    ...
    'reservation candidate supports BEFORE only': () => (
      parseBulkWriteVariant('BEFORE', BENCHMARK) === 'BEFORE'
        && rejects(() => parseBulkWriteVariant('AFTER', BENCHMARK))
    ),
